...
fn main() {
    ...
    let interfaces_name_match = |iface: &NetworkInterface| iface.name == "enp0s8";
    let interface = interfaces.into_iter().filter(interfaces_name_match).next().unwrap();
    let source_mac = interface.mac_address();

    // send_l2_packet(interface, source_mac, MacAddr::new(0x08, 0x00, 0x27, 0x63, 0x9d, 0xc2), String::from("Hello").as_bytes());

    // L3 packet
    let source_ip: Ipv4Addr = "172.28.128.3".parse().unwrap();
    let target_ip: Ipv4Addr = "10.0.2.15".parse().unwrap();
    send_ip_packet(interface, source_mac, MacAddr::new(0x08, 0x00, 0x27, 0x63, 0x9d, 0xc2), source_ip, target_ip, String::from("Hello IP!").as_bytes());
